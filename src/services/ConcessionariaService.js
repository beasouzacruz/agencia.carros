...
export class ConcessionariaService {
    ...
    async cadastrar(nome, cnpj, cidade) {
        ...
        return await this.repository.salvar(
            new Concessionaria(nome, cnpj, cidade)
        );
    }
    ...
    async atualizar(id, nome, cnpj, cidade) {
        if (!id)
            throw new Error("Id é obrigatória para atualização.");
        ...
        const concessionariaAtualizada =
            new Concessionaria(nome, cnpj, cidade, id);

        return await this.repository.atualizar(
            id,
            concessionariaAtualizada
        );
    }
